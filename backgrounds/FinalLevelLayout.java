...
public class FinalLevelLayout implements Background{
	...
	public FinalLevelLayout() {
		...
		map = CSVReader.importFromCSV("res/CSVMaps/FinalLevel.csv");
	...
	public ArrayList<DisplayableSprite> getMappedSprites() {
		...
		for (int row = 0; row < map[0].length; row++) {
			for (int col = 0; col < map.length; col++) {
				...
				if (map[col][row] == 5) {
					sprites.add(new ToggleBox(row * TILE_WIDTH, col * TILE_HEIGHT, (row + 1) * TILE_WIDTH, (col + 1) * TILE_HEIGHT, true));
				}
			}
		}
		return sprites;

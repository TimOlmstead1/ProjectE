...
public class FinalUniverse implements FightingUniverse {
	...
	private ArrayList<DisplayableSprite> boxes = new ArrayList<DisplayableSprite>();
	...
	public FinalUniverse () {

		this.setXCenter(256);
		this.setYCenter(208);
		
		background = new RepeatedStonyWall(1);
		foreground = new FinalLevelLayout();
	...
	public void update(KeyboardInput keyboard, MouseInput mouse, long actual_delta_time) {
		...
		if ((keyboard.keyDownOnce(27))||(getIsPlayer1Dead())) {
			complete = true;
		}
		...
		if (!(fightStarted)) {
			...
			for (int i = 0; i < boxes.size(); i++) {
				readyToStart = ((ToggleBox)boxes.get(i)).getActivity();
				if (!(readyToStart)) {
					break;
				}
			}
	...
	public void startFight() {
		levelBoss = new FinalBoss(StandardLevelLayout.TILE_WIDTH * 32, StandardLevelLayout.TILE_HEIGHT * 47);
	...
	public boolean getIsPlayer1Dead() {
		return ((RangerCharacterSprite) player1).getAlive();
	}
...
//	private void collectToggleBox(Universe universe) {
//		
//		int i = 0;
//		
//		for (DisplayableSprite sprite : universe.getSprites()) {
//			if (sprite instanceof ToggleBox) {
//				boxes[i] = sprite;
//				i++;
//			}
//		}

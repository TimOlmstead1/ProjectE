...
public class EyeEnemy implements EnemySprite, MovableSprite{
	
	private static final int WIDTH = 19;
	private static final int HEIGHT = 27;
	private static final double RESULTANT_VELOCITY = 50;
	...
	private static Image[] eyeSprites = null;
	...
	public EyeEnemy(double centerX, double centerY) {
		...
		if (eyeSprites == null) {
			try {
				eyeSprites = new Image[4];
				for (int i = 1; i <= eyeSprites.length; i++) {
					String path = String.format("res/Eye/eye%d.png", i);
					eyeSprites[i-1] = ImageIO.read(new File(path));
				}
			}
			catch (IOException e) {
